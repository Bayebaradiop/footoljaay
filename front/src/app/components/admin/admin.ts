...
export class AdminComponent implements OnInit {
  ...
  approveProduct(product: Product): void {
    if (confirm(`Approuver le produit "${product.title}" ?`)) {
      this.loading.set(true);
      this.apiService.approveProduct(product.id).subscribe({
        next: (approvedProduct) => {
          console.log('✅ Produit approuvé:', approvedProduct);
  ...
  rejectProduct(product: Product): void {
    const reason = prompt(`Rejeter le produit "${product.title}"?\n\nRaison du rejet (optionnel):`);
    if (reason !== null) { // null = annulation, chaîne vide = ok sans raison
      this.loading.set(true);
      this.apiService.rejectProduct(product.id, reason || undefined).subscribe({
        next: (rejectedProduct) => {
          console.log('✅ Produit rejeté:', rejectedProduct);
